...
/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Timer_ADC_1_Start(void) 
{
    /* Set the bit to enable the clock. */
    Clock_Timer_ADC_1_CLKEN |= Clock_Timer_ADC_1_CLKEN_MASK;
	Clock_Timer_ADC_1_CLKSTBY |= Clock_Timer_ADC_1_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Timer_ADC_1_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Clock_Timer_ADC_1_CLKEN &= (uint8)(~Clock_Timer_ADC_1_CLKEN_MASK);
	Clock_Timer_ADC_1_CLKSTBY &= (uint8)(~Clock_Timer_ADC_1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Timer_ADC_1_StopBlock(void) 
{
    if ((Clock_Timer_ADC_1_CLKEN & Clock_Timer_ADC_1_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_Timer_ADC_1__CFG3)
        CLK_DIST_AMASK = Clock_Timer_ADC_1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Timer_ADC_1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Timer_ADC_1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Timer_ADC_1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Timer_ADC_1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Timer_ADC_1_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_Timer_ADC_1_CLKSTBY &= (uint8)(~Clock_Timer_ADC_1_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Timer_ADC_1_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_Timer_ADC_1_GetSourceRegister();
    uint16 oldDivider = Clock_Timer_ADC_1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Timer_ADC_1_CLKEN & Clock_Timer_ADC_1_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_Timer_ADC_1_DIV_PTR, clkDivider);
                Clock_Timer_ADC_1_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_Timer_ADC_1_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_Timer_ADC_1_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Timer_ADC_1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Clock_Timer_ADC_1_DIV_PTR, clkDivider);
				Clock_Timer_ADC_1_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Clock_Timer_ADC_1_GetDividerRegister(void) 
{
    return CY_GET_REG16(Clock_Timer_ADC_1_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Timer_ADC_1_SetModeRegister(uint8 modeBitMask) 
{
    Clock_Timer_ADC_1_MOD_SRC |= modeBitMask & (uint8)Clock_Timer_ADC_1_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Timer_ADC_1_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_Timer_ADC_1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Timer_ADC_1_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_Timer_ADC_1_GetModeRegister(void) 
{
    return Clock_Timer_ADC_1_MOD_SRC & (uint8)(Clock_Timer_ADC_1_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Timer_ADC_1_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_Timer_ADC_1_GetDividerRegister();
    uint8 oldSrc = Clock_Timer_ADC_1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Timer_ADC_1_MOD_SRC |= CYCLK_SSS;
        Clock_Timer_ADC_1_MOD_SRC =
            (Clock_Timer_ADC_1_MOD_SRC & (uint8)(~Clock_Timer_ADC_1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_Timer_ADC_1_GetSourceRegister(void) 
{
    return Clock_Timer_ADC_1_MOD_SRC & Clock_Timer_ADC_1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Timer_ADC_1_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_Timer_ADC_1_PHASE = clkPhase & Clock_Timer_ADC_1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Timer_ADC_1_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_Timer_ADC_1_GetPhaseRegister(void) 
{
    return Clock_Timer_ADC_1_PHASE & Clock_Timer_ADC_1_PHASE_MASK;
}
