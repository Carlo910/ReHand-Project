...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_BT) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)))


/*******************************************************************************
* Function Name: UART_BT_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_BT_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_BT_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_BT_CyBtldrCommWrite
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    while(bufIndex < size)
    {
        UART_BT_PutChar(pData[bufIndex]);
        bufIndex++;
    }
...
/*******************************************************************************
* Function Name: UART_BT_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
        ...
        */
        if(0u != UART_BT_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = UART_BT_GetRxBufferSize();
                CyDelay(UART_BT_BYTE2BYTE_TIME_OUT);
            }
            while(UART_BT_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  UART_BT_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(UART_BT_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(UART_BT_GetRxBufferSize() > 0u)
    {
        tempCount = UART_BT_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = UART_BT_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional UART_BT_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != UART_BT_PACKET_EOP)
            {
                CyDelay(UART_BT_WAIT_EOP_DELAY);
            }
        }
